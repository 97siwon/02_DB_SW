...
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
WHERE (JOB_CODE, DEPT_TITLE) = (SELECT JOB_CODE, DEPT_TITLE
                                FROM EMPLOYEE
                                WHERE EMP_NAME = '노옹철')
...
SELECT EMP_ID , EMP_NAME , NVL(DEPT_TITLE, '부서없음'), JOB_NAME, HIRE_DATE 
FROM EMPLOYEE MAIN
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON(DEPT_ID = DEPT_CODE)
WHERE ENT_YN = 'N'
AND HIRE_DATE = (SELECT MIN(HIRE_DATE) /* 2번 */
                 FROM EMPLOYEE SUB
                 WHERE SUB.DEPT_CODE = MAIN.DEPT_CODE)
                                           /* 1번 */
ORDER BY HIRE_DATE;

-- 1번) MAIN 쿼리 1행의  DEPT_CODE를 서브쿼리에 대입
-- 2번) SUB 쿼리를 수행
-- 3번) SUB 쿼리의 결과를 이용해서 MAIN의 조건절을 수행

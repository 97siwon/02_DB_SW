...
--  ORA-12899: "KH_LSW"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
...
CREATE TABLE DEPT_COPY 
AS SELECT * FROM DEPARTMENT;
--> 컬럼명, 데이터 타입, NOT NULL여부만 복사

-- DEPT_COPY 테이블에 PK 추가(컬럼 : DEPT_ID, 제약조건명 : D_COPY_PK)
ALTER TABLE DEPT_COPY ADD CONSTRAINT C_COPY_PK PRIMARY KEY(DEPT_ID);

-- 3. 이름 변경(컬럼, 제약조건건, 테이블)
...
ALTER TABLE DEPT_COPY RENAME CONSTRAINT C_COPY_PK TO DEPT_COPY_PK;
...
-- DROP TABLE 테이블명 [CASCADE COMSTRAINTS];
...
--무결성 제약조건(KH_LSW.SYS_C008392)이 위배되었습니다- 부모 키가 없습니다
...
--> 해결방법
-- 1) 자식, 부모 테이블 순서로 삭제
-- 2) ALTER을 이용해서 FK 제약조건 삭제 후 TB1 삭제
...
-- 컬럼명 변경 DDL
ALTER TABLE TB2 RENAME COLUMN TB2_COL TO TB2COLCOL;

...
-- 함수는 SELECT문의
-- SELECT절, WHERE절, ORDER BY절, GROUP BY 절, HAVING절 사용 가능
...
-- 지정한 위치로부터 지정한 순번째로 검색되는 문자의 위치를 반환
...
SELECT ROUND(123.456, -1) FROM DUAL; -- 소수점 0 번째 자리에서 반올림
                                     -- 소수점 -1 자리 표현
...
SELECT LAST_DAY('2026-02-01') FROM DUAL;
...
SELECT SYSDATE FROM DUAL; -- 2022-09-01 11:20:43.000

-- 2022/09/01 11:20:43 목요일
...
-- EMPLOYEE 테이블에서 각 지원이 태어난 생년월일 조회(1990년 05월 13일) 조회
SELECT EMP_NAME, 
...
-- 직원의 급여를 인상하고자 한다.
-- 직급 코드가 J7인 사람은 20% 인상,
-- 직급 코드가 J6인 사람은 15% 인상,
-- 직급 코드가 J5인 사람은 10% 인상,
-- 그 외 직급은 5% 인상
...
-- AVG(숫자가 기록된 컬럼명)  : 평균
-- 전 직원 급여 평균
SELECT ROUND( AVG(SALARY) ) FROM EMPLOYEE; -- 3047662.60869565217391304347826086956522
...
-- 급여 최소값, 가장 빠른 입사일, 이메일 알파멧 순서가 가장 빠른 이메일
SELECT MIN(SALARY), MIN(HIRE_DATE), MIN(EMAIL)
FROM EMPLOYEE;

-- 급여 최대값, 가장 느린 입사일, 이메일 알파멧 순서가 가장 느린 이메일

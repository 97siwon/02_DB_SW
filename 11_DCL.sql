/* 
   ...
   DCL(DATA CONTROL LANGUAGE) : 데이터를 다루기 위한 권한을 다루는 언어
     ...
     - 계정에 DB, DB객체에 대한 권한을 부여(GRANT)하고 회수(REVOKE)하는 언어
...
CREATE USER lsw_sample IDENTIFIED BY sample1234;
...
GRANT CREATE SESSION TO lsw_sample;
...
GRANT CREATE TABLE TO lsw_sample;

ALTER USER lsw_sample DEFAULT TABLESPACE
...
GRANT CONNECT, RESOURCE TO lsw_sample;
...
-- kh_lsw      /      lsw_sample 사용자 계정끼리 서로 객체 접근 권한 부여

-- 1. (sample) kh_lsw 계정의 EMPLOYEE 테이블 조회
SELECT * FROM kh_lsw.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다.
--> 접근 권한이 없어서 조회 불가

-- 2. (kh) lsw_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO lsw_sample;

-- 3. (sample) 다시 kh_lsw.EMPLOYEE 조회
SELECT * FROM kh_lsw.EMPLOYEE;
...
-- 4. (sample) kh_lsw.EMPLOYEE 테이블을 복사한 테이블 생성
CREATE TABLE EMP_SAMPLE
AS SELECT * FROM kh_lsw.EMPLOYEE;
...
REVOKE SELECT ON EMPLOYEE FROM lsw_sample;

-- 6. (sample) 권한 회수 확인
SELECT * FROM lsw_sample.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다.

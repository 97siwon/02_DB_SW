...
FROM EMPLOYEE
GROUP BY DEPT_CODE ; --> DEPT_CODE가 같은 행끼리 하나의 그룹이 된다.
-- ORA-00937: 단일 그룹의 그룹 함수가 아닙니다

-- EMPLOYEE 테이블에서 
-- 직급 코드가 같은 사람의 직급 코드, 급여 평균, 인원 수를 
-- 직급 코드 오름 차순으로 조회
...
SELECT DECODE( SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여' ) 성별,
	...
	SUM(SALARY) "급여 합"
FROM EMPLOYEE;
GROUP BY DECODE( SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여' ); -- 별칠 사용X (별칠 해석X)
...
FROM EMPLOYEE
-- WHERE SALARY >= 3000000 --> 한 사람의 급여가 300백만원 이상이라는 조건(요구사항X)
GROUP BY DEPT_CODE 
